import asyncio
import os
from logging.config import fileConfig
from sqlalchemy import pool
from sqlalchemy.engine import Connection
from sqlalchemy.ext.asyncio import create_async_engine
from alembic import context

# Configure Python path if needed (we run Alembic from repo root)
import sys
from pathlib import Path
sys.path.append(str(Path(__file__).resolve().parents[1] / "src"))

# App metadata
from profile_service.repo.sql.models import Base  # target_metadata
target_metadata = Base.metadata

# Alembic Config
config = context.config
if config.config_file_name is not None:
    fileConfig(config.config_file_name)

# Prefer explicit ALEMBIC_DATABASE_URL (sync driver), fallback to app async URL
ALEMBIC_DATABASE_URL = os.getenv("ALEMBIC_DATABASE_URL")
ASYNC_URL = os.getenv("DATABASE_URL")  # optional, if you keep one source of truth

def run_migrations_offline() -> None:
    url = ALEMBIC_DATABASE_URL or (ASYNC_URL and ASYNC_URL.replace("+asyncpg", ""))
    assert url, "Set ALEMBIC_DATABASE_URL or DATABASE_URL"
    context.configure(
        url=url,
        target_metadata=target_metadata,
        literal_binds=True,
        dialect_opts={"paramstyle": "named"},
    )
    with context.begin_transaction():
        context.run_migrations()

def do_run_migrations(connection: Connection) -> None:
    context.configure(connection=connection, target_metadata=target_metadata)
    with context.begin_transaction():
        context.run_migrations()

async def run_async_migrations() -> None:
    assert ASYNC_URL, "Set DATABASE_URL for async migrations"
    engine = create_async_engine(ASYNC_URL, poolclass=pool.NullPool)
    async with engine.connect() as conn:
        await conn.run_sync(do_run_migrations)
    await engine.dispose()

def run_migrations_online() -> None:
    if ALEMBIC_DATABASE_URL:
        # Use sync connection path (psycopg)
        from sqlalchemy import create_engine
        connectable = create_engine(ALEMBIC_DATABASE_URL)
        with connectable.connect() as connection:
            do_run_migrations(connection)
        connectable.dispose()
    else:
        asyncio.run(run_async_migrations())

if context.is_offline_mode():
    run_migrations_offline()
else:
    run_migrations_online()

